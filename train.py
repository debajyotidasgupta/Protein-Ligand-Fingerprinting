...
def generate_neighbour_fingerprints():
    ...
    """
    # if intermediate directory does not exist, create it
    if not os.path.isdir(intermediate_dir):
        os.mkdir(intermediate_dir)

    # if fingerprints directory does not exist, create it
    ...
    for pos, pdb_id in tqdm(list(enumerate(pdb_ids))):
        if pos > max_pdbs:
            break
...
def train(input_dim=5, hidden_dim=32, num_layers=2, num_heads=5, output_dim=256, epochs=10):
    ...
    fingerprints_train, fingerprints_test, _, _ = train_test_split(
        fingerprints, [0 for x in fingerprints], test_size=0.2, random_state=random_seed)
        ...
        for i, data in enumerate(test_loader, 0):
            ...
            if loss < min_loss:
                ...
                torch.save(model, os.path.join(
                    models_dir, f"AutoencoderTransformer.pt"))
...
if __name__ == "__main__":
    ...
    train(args.input_dim, args.hidden_dim, args.num_layers,
          args.num_heads, args.output_dim, args.epochs)
